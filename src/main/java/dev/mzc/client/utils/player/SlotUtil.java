...
public class SlotUtil {
    ...
    private static int creativeInventory(int i) {
        if (CreativeInventoryScreen.selectedTab != Registries.ITEM_GROUP.get(ItemGroups.INVENTORY)) {
            return -1;
        }
    ...
    private static int horse(ScreenHandler handler, int i) {
        ...
        int horseSlots = totalSlots - 36;
        if (horseSlots < 0) return -1;
        if (isMain(i)) return horseSlots + (i - 9);
        if (isHotbar(i)) return horseSlots + 27 + i;
        return -1;
    }
