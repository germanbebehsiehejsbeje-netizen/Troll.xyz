...
#define iTime time
#define iMouse vec4(resolution * vec2(0.8, 0.4) + vec2(mouse.x, 0.0), 0.0, 0.0)
...
float noise(in vec3 x) {
    ...
    return -1.0 + 2.0 * mix(nxy0, nxy1, f.z);
}
...
vec3 stars(in vec3 p)
{
    ...
    p.x += (tri(p.z*30.)+tri(p.y*30.))*0.004;
    p.y += (tri(p.z*30.)+tri(p.x*30.))*0.004;
    p.z += (tri(p.x*30.)+tri(p.y*30.))*0.004;
...
vec3 getBgColor( vec3 dir ) {
    ...
    vec3 c1 = stars(dir);
    vec3 haze = vec3(0.02, 0.01, 0.05) + 0.02 * sin(vec3(0.0, 2.0, 4.0) + theta + iTime * 0.05);
    return haze + c1*2.0;
}
...
vec4 Radiance( in Ray ray )
{
    ...
    float marchingStep = mix( 0.45, 0.55, rnd() );
    ...
    for(int i=0; i<28 && transmittance > 1e-3; i++) {
